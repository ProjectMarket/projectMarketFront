...
(function() {
  ...
  function CarouselController($timeout, $attrs, $interval, $window) {
    ...
    that.allSentence = ['Les meilleurs professionels pour les meilleurs projets',"Venez discuter de vos idées autour d'un café","Les meilleurs professionels pour les meilleurs projets"];
    that.currentSentence = 0;
    ...
    that.navigateLeft = function() {
      ...
      if(that.currentSentence == 0){
        that.currentSentence = that.allSentence.length-1;
      }else{
        that.currentSentence--;
      }
      that.currentIndex--;
      that.radioButtonIndex = that.currentIndex;
    ...
    that.navigateRight = function(autoSlide) {
      ...
      if(that.currentSentence == that.allSentence.length-1){
        that.currentSentence = 0;
      }else{
        that.currentSentence++;
      }
      console.info("that.currentSentence === ", that.currentSentence);
      that.currentIndex++;
      that.radioButtonIndex = that.currentIndex;
      that.currentMarginLeftValue -= that.currentWidth;
      that.applyMarginLeft();
    ...
    that.onRadioButtonClick = function() {
      ...
      that.currentSentence = that.currentIndex;
      that.applyMarginLeft();
      if (that.autoSlideStopOnAction) { 
        that.stopAutoSlide(); 
      } else {         
...
(function(){angular.module("jkAngularCarousel.templates", []).run(["$templateCache", function($templateCache) {$templateCache.put("carousel-directive.html","<div class=\"jk-carousel\" >\n\n  <div class=\"slides-container\" layout=\"row\" >\n    <div\n      ng-repeat=\"slideItem in ctrl.cloneData\"\n      class=\"slide\"\n    >\n      <div ng-include=\"ctrl.itemTemplateUrl\" ></div>\n    </div>\n  </div>\n\n  <md-button class=\"md-icon-button left-arrow-button\" >\n    <md-icon class='material-icons' ng-click=\"ctrl.navigateLeft()\" >navigate_before</md-icon>\n  </md-button>\n\n   <div layout=\"row\"\n layout-align=\"center center\"\n flex><span hide show-md class=\"slide-text text-slider-md  text-slider-container \" >{{ctrl.allSentence[ctrl.currentSentence]}}</span><span hide show-sm class=\"slide-text text-slider-sm  text-slider-container \" >{{ctrl.allSentence[ctrl.currentSentence]}}</span><span hide show-xs class=\"slide-text text-slider-xs  text-slider-container \" >{{ctrl.allSentence[ctrl.currentSentence]}}</span><span hide show-gt-md class=\"slide-text text-slider-gt-md  text-slider-container \" >{{ctrl.allSentence[ctrl.currentSentence]}}</span><md-button hide-xs class=\"button-slider-gt-sm  button-slider-container \" >\n  Inscrivez-vous</md-button><md-button hide show-xs class=\"button-slider-xs  button-slider-container \" >\n  Inscrivez-vous</md-button></div>\n\n <md-button class=\"md-icon-button right-arrow-button\" >\n    <md-icon class='material-icons' ng-click=\"ctrl.navigateRight()\" >navigate_next</md-icon>\n  </md-button>\n\n  <md-radio-group\n    class=\"radio-buttons-container\"\n    layout=\"row\"\n  ng-model=\"ctrl.radioButtonIndex\"\n    layout-align=\"center center\"\n    ng-change=\"ctrl.onRadioButtonClick()\" >\n    <md-radio-button\n      ng-repeat=\"item in ctrl.data\"\n      ng-value=\"$index\"\n      aria-label=\"$index\" >\n    </md-radio-button>\n  </md-radio-group>\n\n</div>\n");}]);})();