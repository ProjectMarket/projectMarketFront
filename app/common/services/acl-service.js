...
(function() {
  ...
  var objectName = 'pm.common.aclService';
      ...
      .factory(objectName, [
        ...
        function(
          ...
          var _factory = {
             ...
             */
            loadAcl: function() {
              pmLog.trace({message: "Entrée méthode", object: objectName, method: "loadAcl", tag: "methodEntry"});

              var pmBackComHandler = _getpmBackComHandler(),
                  deferred = $q.defer();
              pmBackComHandler.post('session/acl', {})
                  .then(function(data) {
                    try {
                      // rôle fermé
                      if (data[0].UnitResponse.objects[0].ErrorResponse !== undefined
                        ...
                        pmLog.info({message: "Tentative de récupérer les ACL pour un profil fermé.", tag: "acl", object: objectName, method: "loadAcl"});
                        pmRouter.navigateToErrorPage('acl', 'closedProfile');
                      ...
                      pmLog.info({message: "ACL récupérées depuis le backend avec succès. ACL = {{acl}}",
                      ...
                      pmLog.error({message: "Erreur lors de la lecture de l'objet ACL. Message = {{message}}", tag: "acl",
                  ...
                  .catch(function(response) {
                    ...
                    pmLog.error({message: "Erreur lors de la récupération des ACL depuis le backend.", tag: "acl", object: objectName, method: "loadAcl"});
                    deferred.reject("Erreur lors de la récupération des ACL depuis le backend.");
                    // redirection vers page d'erreur
                    pmRouter.navigateToErrorPage('acl', 'load');
                  });
            ...
            isAllowedAccessToComponent: function(component) {
              ...
              pmLog.trace({message: "Entrée méthode", object: objectName, method: "isAllowedAccessToComponent", tag: "methodEntry"});
              pmLog.debug({message: "Paramètres méthode : {{params}}",
                params: {params: arguments}, tag: "params", object: objectName, method: "isAllowedAccessToComponent"});

              try {
                var componentSecurityConfig = pmComponent.getComponentSecurityConfig(component);
              } catch (e) {
              ...
              if (!componentSecurityConfig.isProtected) {
                pmLog.debug({message: "Component {{component}} non protégé.",
              ...
              if (componentSecurityConfig.crudObjects.length === 0) {
                pmLog.debug({message: "Pas de crudObject affecté au component {{component}}.",
                  params: {component: component}, tag: "ACL", object: objectName, method: "isAllowedAccessToComponent"});
                _isAllowedAccessToComponentCache[component] = true;
                return true;
              }
              ...
              for (var i = 0; i < componentSecurityConfig.crudObjects.length; i++) {
                if (_acl[componentSecurityConfig.crudObjects[i].name] === undefined) {
                  pmLog.debug({message: "CrudObject non trouvé dans les ACL : {{crudObject}}.",
                ...
                if (_acl[componentSecurityConfig.crudObjects[i].name][componentSecurityConfig.crudObjects[i].action] === true) {
                  pmLog.debug({message: "Accès au component {{component}} autorisé par les ACL pour le crudObject {{crudObject}}.",
              ...
              pmLog.info({message: "Accès au component {{component}} refusé par les ACL.",
                params: {component: component}, tag: "ACL", object: objectName, method: "isAllowedAccessToComponent"});
              _isAllowedAccessToComponentCache[component] = false;
              return false;
            ...
            isAllowedManageCrudObject: function(crudObject, action) {
              pmLog.trace({message: "Entrée méthode", object: objectName, method: "isAllowedManageCrudObject", tag: "methodEntry"});
              pmLog.debug({message: "Paramètres méthode : {{params}}",
              ...
              if (action === 'READ' || action === 'CREATE') {
                if (typeof crudObject !== 'string') {
                  pmLog.error({message: "Pour l'action {{action}}, le crudObject doit être de type String, crudObject fourni : {{crudObject}}.",
                  ...
                  pmLog.error({message: "ACL inconnue pour l'action {{action}} sur le crudObject {{crudObject}}.",
                    params: {action: action, crudObject: crudObject}, tag: "ACL", object: objectName, method: "isAllowedManageCrudObject"});
                ...
                if (typeof crudObject !== 'object') {
                  pmLog.error({message: "Pour l'action {{action}}, le crudObject doit être de type Object, crudObject fourni : {{crudObject}}.",
                    params: {action: action, crudObject: crudObject}, tag: "ACL", object: objectName, method: "isAllowedManageCrudObject"});
                ...
                pmLog.error({message: "Le paramètre action doit avoir comme valeur READ|CREATE|UPDATE|DELETE, la valeur fournie est {{action}}.",
                  params: {action: action}, tag: "ACL", object: objectName, method: "isAllowedManageCrudObject"});
              }

              if (isAllowed) {
                pmLog.debug({message: "Autorisation accordée par les ACL pour l'action {{action}} sur le crudObject {{crudObject}}.",
                  params: {action: action, crudObject: crudObject}, tag: "ACL", object: objectName, method: "isAllowedManageCrudObject"});
              } else {
                pmLog.debug({message: "Autorisation refusée par les ACL pour l'action {{action}} sur le crudObject {{crudObject}}.",
                  params: {action: action, crudObject: crudObject}, tag: "ACL", object: objectName, method: "isAllowedManageCrudObject"});
              }
